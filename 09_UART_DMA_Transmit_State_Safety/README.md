...
User presses button on PC13 (debounced via 50 ms filter).

Firmware verifies dma_ready == 1 before toggling motor state on PA0.

Firmware raises lock (dma_ready = 0) and dispatches the corresponding state string over DMA.

CPU continues execution immediately without waiting for serial transmission latency.

Upon transfer completion, the DMA hardware invokes HAL_UART_TxCpltCallback, restoring dma_ready = 1.
